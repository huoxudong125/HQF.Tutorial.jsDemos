...
function pullDownAction(tabContentTag) { //下拉事件  
    setTimeout(function () {
        var el, li, i;
        el = $(tabContentTag+ ' ul');
        for (i = 0; i < 3; i++) {
...
function pullUpAction( tabContentTag) { //上拉事件  
    setTimeout(function () {
        var el, li, i;
        el = $(tabContentTag+ ' ul');
        for (i = 0; i < 3; i++) {
...
function loaded(tabContentTag) {
    pullDownEl = $(tabContentTag+' .pullDown');
    pullDownL = pullDownEl.find('.pullDownLabel');
    pullDownEl['class'] = pullDownEl.attr('class');
    pullDownEl.attr('class', '').hide();

    pullUpEl = $(tabContentTag + ' .pullUp');
    pullUpL = pullUpEl.find('.pullUpLabel');
    pullUpEl['class'] = pullUpEl.attr('class');
    pullUpEl.attr('class', '').hide();

    myScroll = new IScroll(tabContentTag, {
